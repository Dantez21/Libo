$(document).ready(function () {
    ...
    if (partnerID > -1)
    {
        $("#partner_form input, #partner_form textarea, #partner_form select").prop("disabled", true);
        $("#btn-save").hide();

        $("#btn-edit").click(function () {
            $("#btn-save").show();
            $(this).hide();
            $("#partner_form input, #partner_form textarea, #partner_form select").prop("disabled", false);
        });

    } 
    
    $("#latlng, #physical_address").prop("readonly", true);
...
function init_profile_uploader()
{
    ...
    var uploader = new plupload.Uploader({
        ...
        url: $("#site_url").val() + '/partners/upload_cover_image',
        unique_names: true,
        resize: {width: 500, height: 300, quality: 300, crop : true},
        max_retries: 3,
        flash_swf_url: 'Moxie.swf',
        silverlight_xap_url: 'Moxie.xap',
        multipart_params: {
            partnerID: $("#pickfiles").data("partner-id"),
        ...
        init: {
            ...
            UploadComplete: function (up, files) {
                plupload.each(files, function (file) {
                    var src = BASE_URL + "/uploads/partner-" + $("#pickfiles").data("partner-id") + "/" + file.target_name;
                    $("#img-pic").attr("src", src);
                    $("#partner_imageURL").val(file.target_name);
                });
            },
...
function init_food_image_uploader()
{
    ...
    var uploader_f = new plupload.Uploader({
        ...
        url: $("#site_url").val() + '/partners/upload_food_image',
        unique_names: true,
        resize: {width: 500, height: 300, quality: 300, crop : true},
        max_retries: 3,
        flash_swf_url: 'Moxie.swf',
        ...
        init: {
            ...
            UploadComplete: function (up, files) {
                plupload.each(files, function (file) {
                    var src = BASE_URL + "/uploads/partner-" + $("#partnerID").val() + "/" + file.target_name;
...
function init_attachment_uploader()
{
    ...
    var uploader_a = new plupload.Uploader({
        ...
        url: $("#site_url").val() + 'partners/upload_attachment',
        unique_names: true,
