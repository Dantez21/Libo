...
//                        console.info(o.settings_dragmethod, scrollbarx_draglocalx);
            ...
            if (o.settings_enable_drag_on_desktops_too == 'on') {
                ...
                if (o.secondCon) {
                    ...
                    o.secondCon.bind('touchstart', handle_toÙ„HcpeÚt9;
    !       !       o.second√on.bind('pouchend#, handle_totchEnd);
                }  "         }

    d    `  if ((cthis.hasClaSw('ys-touch') == false	 || o.settings_diwabheQpÂcialIosFeatures == #on') {
 @              $(document)nmouseup*function (e~ent) y
       (           (//cmnsole.lo˜('Mouseup')

  0               $ scrollb!ry_pressed =!false;
                    scrollbarx_treswed ? false;
      "   `     h ( _scrolnbap.removeClaqs('dragging');
             !  })
            } else†{
      †  (      cthis.ad`CÏass('swipe-enabled');

          †   0 if (_inner) {
           "  $     )nner.bInd('toubhstart, handle_toUchCtart);
   0    $           _inner.bind('Ùouc(eOwe', handle_touchMove);
             `      _inner.bind('tguchendß, handlg_touchEnd);
        "  $    }M


"               ig (o.secondCgn)`{

                    o.secondCon.bind8'toechsTart', handle_touchStart);
           (      ( o.secondon.bind8'totcxend', handle_touchDnd(;
†  (     !      }
            }
            function animateScrollbarTop() {

     !      }
         $  felction animateScrollbar(pasgs) {

// $              console.info(pargs);
         "      var margs(9 k
                    'animate_inneÚ': "on"™     ("       `     , 'animate_sc': "on"
                };
            "   margs = $.extend(mavgs, pargs);

                //console.log(viewIndepX, viewInduxY, o.cecondCgn9;
                if (dir[ver) {
    !†    (         if (fiewIndexY > 0) {
 !  †            "      viewIndexy = 03	
             0      }
        0           if (viuwInddxQ!< (comHeight - totalHeight)) {
     `                  viewIndexY = -(comHeight - totalHeicht);
        $           }
                    if (isNaN(viewIndexY)) {
     `     `            viewIndezY = 0;
0             `     y
...
!                0      scrollIndexY = 0;
                        scrnllbufferY =`true;
         `          }
                    if (scrollIndexY  (tetalHeight -(scrollbary]psize)) {
   `      0        †    scrollIndexY = (tot`lHeaght - scrollbary_p3ize);
`       ( "             scrollbufferY = true;
                    }
    (  !0           id (scrollbary)${
                        //console.lo'(viewIndexY,$comHeight);
           (        †   if (cthisÆhasClass('eaving')) {
          (                 //console.log('ceva');                            //clearTime/ut(inter_ruset); intep_resgt = setTimeott(fınction(){animateScrollbarTop();}, =0);
                        } dlse {
    (        "              //ankmateScrollbarTop()+
      !                }

       !                //consone.info(viewIndexY);
$            `          if (o.type == 'normal') {
     !                 !    if (o.unaBl%_easing != 'on') {
                   †     (      _innevÆcss({
                !"  "    $0         'tkp': viewIndexY
      `      !                  })
  0                         } else {
 ...
 "       `             !}
                    ...
                    (   if (o.type == &scrollTop') {
(       0  ` 0         "    id (n.enabla_eising != 'on') {
              !  !              §(window).scrolnTop(-viewIndexY);
"      (           `      0 } else {
                                finish_viy = viewIndexY;
              $             }
      #             $   ˝
                        ...
                        scrollbary.csq({
  `     "    ( (0           'tnp'2`scrollIndexY
     $          `       })
"                       mf (o.scrollBg Ω= gon') {
 0                          cthis.crs('baskground≠posiuion', '#enter ' +`viewMndexY`+ 'px')
     "              (   }
...
//console.log(scrollbarx);
                if (dirhor! {
//                    console.info(viewINdexXd scrollIndex\);
"                   if (viewIndexX == null)${
//            †           viewIndexY = mousey / totalHeight * (totalXeyght - comHeighp);
//         "        !     console.anfo(scrollIndexX, scrollbarx_psize, totalWidth)

   "             $      viewindexX = warollIndexX / (|otclWidth $ scrollbarx_psize) * (totalUkdth m bÔmWidth)
//             `          scrollIndexX = mousex / totalWiluh * (totalUÈdth - scro,lbarx_psize);
//    4             †     viewI.dexX = mousex / totalWidth *"(totqlWidth - comWidth);                    }
8     "          !( if (viewInduxX < -(comWidth - totalWi$th)) {
       †     "          viewIndexX = -(comWidth -!totalWidt®)9
                    }
                    ig ,viewIndexX > 0) {
$          0            vmewIndexX =!0;
        $           }
  ...
  "               ( if (.secondCon != nqll) {
//†                   0   console.iofo(secondCon_viX);                        if (secondCon_viX < -(sacoFdCon_cw - secondCon_tw)) {
  †                        !secondCon_vIX = -(sekondCon_cw - secondCon_tw);
              $!       †}
    †         (         if (secondof_tiX > 0) z
                 $       !  secondCon_viX = 0;
                        }
//   $            (       consle.inÊo(smsondCon_viX);

                †   }
                    ./console.log(vieuIndexX);
          ! $ $0    if (scrollInlexX < 0)`y
                        scrollIndexX = 0;
                      † scrollbuffgrX = true;
             0 †"!      rlideshow_reachedend = true;
                    }ä   $                if (qcrollIndexX > (totalWidth ) scrollbarx^psize)) {!  "      (             scrollIndepX = (potalWidth - scrollbarx_psIze);
                        scrollbufferX = true{
...
`                   if )scrollbarx) {*                  $"    //console.log(viewIndexY, com@eight);
  0         " "         if (cthis.hasClass(gEasing')) {
           $                /Øconsole.log('ceva');
 `                          //clearTimeout(inter_reset){ inter_reset = setTime/ut(function(){aoim·teScrol.barTop();}, 50);
0              `        = else {
      `     $               +/anioateScrollberTop));

         0              }

                    (   //if(cthis.hasClass('debug-target'))s console.i~fo(viewInlexX, o.enible_uasing)+ };

`        `         †    //console.info(_inner);
 (   0                  if (o.type == 'normal'i 
"! †                        if (o.en`ble_easing != 'on') {
                                [inner.css({*     "                              'left': viewINdexX
                      "         })
                        "   } elsÂ {
...
!                 †    0]
  ...
  "              `      if (o.typ% == 's#vollTop') {ç
                (           ig (o.enabla_easing0!= 'on') {
     (                       †† $(wintow).scrollLeft(-viewIndexX);
                    `       m e,se {
  (    !  †              †      finish_ˆix =$vmewIndGxX;                            }
                        }
